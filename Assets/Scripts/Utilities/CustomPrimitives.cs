...
public class CustomPrimitives
{
    ...
    private static Mesh Create3DEgg()
    {
        ...
        float height = 2f;
        float eggWaistRadius = 1.6f; //a
        float eggRatio = 6;  //ratio of top height to bottom height. Eg. eR = 4 means waist is at 20% height           
        ...
        float vertsPerCircle = 300; //j
        float circleCount = 30; //n       minmum 3 for diamond shape, 6 will be blocky, 20?
        ...
        //mesh math in here:
        //foreach row
        for (float b = 0; b < rowCount; b++) 
        {
            Debug.Log("START OF ROW #:" + (b+1));
            //CALCULATE RADIUS FOR BOT OF ROW AND RADIUS FOR TOP OF ROW
            float rowBottomHeight = b * (eggBottomLength + eggTopLength) / rowCount; //height from top of egg (going top down apparently)
            Debug.Log("rowBotHeight:" + rowBottomHeight);
            float rowTopHeight = (b + 1) * (eggBottomLength + eggTopLength) / rowCount; //ERROR !!!!!!!!!!
            if (b == 0)
            {
                //FLAT BOTTOM ADJUSTMENT (also b --> b -1 and b+1 --> b above)
                //rowTopHeight = rowBottomHeight;
                rowBottomHeight = rowTopHeight;
            }
